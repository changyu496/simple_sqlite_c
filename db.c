...
{
    memcpy(&(destionation->id), source + ID_OFFSET, ID_SIZE);
    memcpy(&(destionation->username), source + USERNAME_OFFSET, USERNAME_SIZE);
    memcpy(&(destionation->email), source + EMAIL_OFFSET, EMAIL_SIZE);
}
...
void pager_flush(Pager *pager, uint32_t page_num, uint32_t size)
{
    ...
    ssize_t bytes_written = write(pager->file_descriptor, pager->page[page_num], size);    
    if (bytes_written == -1)
    {
